...
class StatusCommand(BotCommand):
    ...
    def _collect_status(self, config) -> dict:
        ...
        status["search_bing_news"] = getattr(config, "bing_news_search_enabled", True)
        status["search_google_news"] = getattr(config, "google_news_search_enabled", True)
    ...
    def _format_status(self, status: dict, platform: str) -> str:
        ...
        lines.extend([
            ...
            f"• Bing News: {icon(status['search_bing_news'])}",
            f"• Google News: {icon(status['search_google_news'])}",
            "",
            "**📢 通知渠道**",
